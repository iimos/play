...
# только отдельные индексы
go run main.go load-index --index IMOEX,MOEXBMI
...
Флаги: `--force` (перезалить), `--start`, `--end`, `--index`, `--watch`.
Последняя дата в таблице перезаливается автоматически. `--index` обновляет
только указанные индексы и не затирает данные остальных (по умолчанию — все).
Список всех индексов берётся из ISS analytics
(`/iss/statistics/engines/stock/markets/index/analytics.json`, блок `indices`) —
это индексы, по которым публикуются веса. Можно явно указать
любой другой код индекса (например, из борда `SNDX`), даже если весов по нему нет.
