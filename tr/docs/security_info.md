...
| `stock_bonds` | облигации | `SU26218RMFS6` (ОФЗ), `RU000A0JQAM6` (корп.) |
...
в `security_info`, пока не перезапущен `load-securities`. Облигации — исключение:
они берутся из полного листинга, поэтому всегда актуальны. Список «отсутствующих»
можно получить так:
