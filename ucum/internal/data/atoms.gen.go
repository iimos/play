...
var Atoms = map[string]types.Atom{
	...
	"[IR]":            {Code: "[IR]", Kind: "amount of an allergen callibrated through in-vivo testing using the Stallergenes® method.", Metric: false, Magnitude: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0x1}), (&big.Int{}).SetBytes([]byte{0x1}))},
	"[BAU]":           {Code: "[BAU]", Kind: "amount of an allergen callibrated through in-vivo testing based on the ID50EAL method of (intradermal dilution for 50mm sum of erythema diameters", Metric: false, Magnitude: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0x1}), (&big.Int{}).SetBytes([]byte{0x1}))},
