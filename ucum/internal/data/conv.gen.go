...
import "math/big"
import "github.com/iimos/play/ucum/internal/types"
...
var Conv = map[string]*types.Unit{
	...
	// U = 10036894500000000⋅s⁻¹
	"U": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0x23, 0xa8, 0x80, 0x9b, 0x52, 0x11, 0x0}), (&big.Int{}).SetBytes([]byte{0x1})), Components: map[types.ComponentKey]int{types.ComponentKey{AtomCode: "s", Annotation: ""}: -1}},
	// V = 1000⋅m²⋅g⋅C⁻¹⋅s⁻²
	...
	// [G] = 667259/10000000000000000000⋅m³⋅g⁻¹⋅s⁻²
	"[G]": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0xa, 0x2e, 0x7b}), (&big.Int{}).SetBytes([]byte{0x8a, 0xc7, 0x23, 0x4, 0x89, 0xe8, 0x0, 0x0})), Components: map[types.ComponentKey]int{types.ComponentKey{AtomCode: "g", Annotation: ""}: -1, types.ComponentKey{AtomCode: "m", Annotation: ""}: 3, types.ComponentKey{AtomCode: "s", Annotation: ""}: -2}},
	// [HPF] = 1
	...
	// [e] = 160217733/1000000000000000000000000000⋅C
	"[e]": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0x9, 0x8c, 0xba, 0x85}), (&big.Int{}).SetBytes([]byte{0x3, 0x3b, 0x2e, 0x3c, 0x9f, 0xd0, 0x80, 0x3c, 0xe8, 0x0, 0x0, 0x0})), Components: map[types.ComponentKey]int{types.ComponentKey{AtomCode: "C", Annotation: ""}: 1}},
	...
	// [h] = 13252151/20000000000000000000000000000000000000⋅m²⋅g⋅s⁻¹
	"[h]": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0xca, 0x36, 0x37}), (&big.Int{}).SetBytes([]byte{0xf, 0xb, 0xdc, 0x21, 0xab, 0xb4, 0x8d, 0xb2, 0x1, 0xe8, 0x6d, 0x40, 0x0, 0x0, 0x0, 0x0})), Components: map[types.ComponentKey]int{types.ComponentKey{AtomCode: "g", Annotation: ""}: 1, types.ComponentKey{AtomCode: "m", Annotation: ""}: 2, types.ComponentKey{AtomCode: "s", Annotation: ""}: -1}},
	// [hd_i] = 127/1250⋅m
	...
	// [k] = 690329/50000000000000000000000000⋅m²⋅g⋅K⁻¹⋅s⁻²
	"[k]": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0xa, 0x88, 0x99}), (&big.Int{}).SetBytes([]byte{0x29, 0x5b, 0xe9, 0x6e, 0x64, 0x6, 0x69, 0x72, 0x0, 0x0, 0x0})), Components: map[types.ComponentKey]int{types.ComponentKey{AtomCode: "K", Annotation: ""}: -1, types.ComponentKey{AtomCode: "g", Annotation: ""}: 1, types.ComponentKey{AtomCode: "m", Annotation: ""}: 2, types.ComponentKey{AtomCode: "s", Annotation: ""}: -2}},
	// [ka'U] = 1
	...
	// [m_e] = 91093897/100000000000000000000000000000000000⋅g
	"[m_e]": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0x5, 0x6d, 0xfb, 0x89}), (&big.Int{}).SetBytes([]byte{0x13, 0x42, 0x61, 0x72, 0xc7, 0x4d, 0x82, 0x2b, 0x87, 0x8f, 0xe8, 0x0, 0x0, 0x0, 0x0})), Components: map[types.ComponentKey]int{types.ComponentKey{AtomCode: "g", Annotation: ""}: 1}},
	// [m_p] = 16726231/10000000000000000000000000000000⋅g
	"[m_p]": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0xff, 0x38, 0xd7}), (&big.Int{}).SetBytes([]byte{0x7e, 0x37, 0xbe, 0x20, 0x22, 0xc0, 0x91, 0x4b, 0x26, 0x80, 0x0, 0x0, 0x0})), Components: map[types.ComponentKey]int{types.ComponentKey{AtomCode: "g", Annotation: ""}: 1}},
	// [mclg'U] = 1
	...
	// eV = 160217733/1000000000000000000000000⋅m²⋅g⋅C⁰⋅s⁻²
	"eV": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0x9, 0x8c, 0xba, 0x85}), (&big.Int{}).SetBytes([]byte{0xd3, 0xc2, 0x1b, 0xce, 0xcc, 0xed, 0xa1, 0x0, 0x0, 0x0})), Components: map[types.ComponentKey]int{types.ComponentKey{AtomCode: "C", Annotation: ""}: 0, types.ComponentKey{AtomCode: "g", Annotation: ""}: 1, types.ComponentKey{AtomCode: "m", Annotation: ""}: 2, types.ComponentKey{AtomCode: "s", Annotation: ""}: -2}},
	// eq = 602213670000000000000000
	"eq": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0x7f, 0x86, 0x11, 0x86, 0xf8, 0x3b, 0x9, 0xf7, 0x0, 0x0}), (&big.Int{}).SetBytes([]byte{0x1})), Components: map[types.ComponentKey]int{}},
	// erg = 1/10000⋅m²⋅g⋅s⁻²
	...
	// kat = 602213670000000000000000⋅s⁻¹
	"kat": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0x7f, 0x86, 0x11, 0x86, 0xf8, 0x3b, 0x9, 0xf7, 0x0, 0x0}), (&big.Int{}).SetBytes([]byte{0x1})), Components: map[types.ComponentKey]int{types.ComponentKey{AtomCode: "s", Annotation: ""}: -1}},
	// l = 1/1000⋅m³
	"l": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0x1}), (&big.Int{}).SetBytes([]byte{0x3, 0xe8})), Components: map[types.ComponentKey]int{types.ComponentKey{AtomCode: "m", Annotation: ""}: 3}},
	// lm = rad²⋅cd
	...
	// mol = 602213670000000000000000
	"mol": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0x7f, 0x86, 0x11, 0x86, 0xf8, 0x3b, 0x9, 0xf7, 0x0, 0x0}), (&big.Int{}).SetBytes([]byte{0x1})), Components: map[types.ComponentKey]int{}},
	// osm = 602213670000000000000000
	"osm": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0x7f, 0x86, 0x11, 0x86, 0xf8, 0x3b, 0x9, 0xf7, 0x0, 0x0}), (&big.Int{}).SetBytes([]byte{0x1})), Components: map[types.ComponentKey]int{}},
	...
	// u = 8302701/5000000000000000000000000000000⋅g
	"u": &types.Unit{Coeff: (&big.Rat{}).SetFrac((&big.Int{}).SetBytes([]byte{0x7e, 0xb0, 0x6d}), (&big.Int{}).SetBytes([]byte{0x3f, 0x1b, 0xdf, 0x10, 0x11, 0x60, 0x48, 0xa5, 0x93, 0x40, 0x0, 0x0, 0x0})), Components: map[types.ComponentKey]int{types.ComponentKey{AtomCode: "g", Annotation: ""}: 1}},
	// wk = 604800⋅s
